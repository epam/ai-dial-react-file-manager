...
module.exports = {
  ...
  /*
   * `shadow-sm` is the side bar / side panel / right panel step, and nothing
   * scanned above paints that surface — the kit's sidebar deliberately draws no
   * background of its own, so the host applies the shadow. Without the safelist
   * the class is scanned out of the shipped stylesheet and a host that writes it
   * against `@epam/ai-dial-react-file-manager/styles.css` gets nothing.
   */
  safelist: ['shadow-sm'],
  theme: {
    backgroundColor: {
    ...
    extend: {
      ...
      boxShadow: {
         ...
         * here: it used to be `sm`, and stayed where it was when `sm` moved onto
         * the side panels, so the kit ships it as the `dial-kit-control-shadow`
         * class rather than as a fifth key.
         */
        xs: `0 1px 4px 0 ${shadowColors['xs-1']}, 0 1px 2px 0 ${shadowColors['xs-2']}`,
        /*
         * sm — Side Bar, Side Panel, right panel; md — Button-Hover,
         * Card-Default, Input; lg — Card-Hover. All three are a single wide blue
         * layer: the grey layer would only muddy it at this size.
         */
