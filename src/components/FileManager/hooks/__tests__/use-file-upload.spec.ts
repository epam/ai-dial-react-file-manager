...
describe('Dial UI Kit :: FileManager :: useFileUpload', () => {
  ...
  it('tracks file dragging over the browser window', () => {
    const { result } = renderUseFileUpload();
    const createWindowDragEvent = (type: string) => {
    ...
    act(() => {
      window.dispatchEvent(createWindowDragEvent('dragenter'));
    });
    expect(result.current.isDraggingOverWindow).toBe(true);

    const dragOverEvent = createWindowDragEvent('dragover');
    act(() => {
    ...
    act(() => {
      window.dispatchEvent(createWindowDragEvent('dragleave'));
    });
    expect(result.current.isDraggingOverWindow).toBe(false);

    act(() => {
      window.dispatchEvent(createWindowDragEvent('dragenter'));
      window.dispatchEvent(createWindowDragEvent('drop'));
    });
    expect(result.current.isDraggingOverWindow).toBe(false);
  });
