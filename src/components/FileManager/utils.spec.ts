...
describe('Dial UI Kit :: file tree helpers', () => {
  it('normalizes file extensions', () => {
    expect(normalizeExtensionWithoutDot('.TXT')).toBe('txt');
  });

  it('collects nested descendants in traversal order', () => {
    const nestedFile = {
      id: 'nested-file',
      name: 'nested.txt',
      path: '/folder/nested/nested.txt',
    ...
    expect(collectAllDescendants(rootFolder)).toEqual([
      nestedFolder,
      nestedFile,
    ]);
  });
